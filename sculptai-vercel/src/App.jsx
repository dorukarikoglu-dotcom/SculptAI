...
FONT_LINK.href = "https://fonts.googleapis.com/css2?family=DM+Serif+Display:ital@0;1&family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,500;9..40,600&display=swap";
document.head.appendChild(FONT_LINK);

/* ─── GLOBAL STYLES ──────────────────────────────────────────────────────── */
const GLOBAL_CSS = `
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'DM Sans',sans-serif;background:#f7f8fa;color:#111827;font-size:13px;line-height:1.5}
...
.f1{animation:fadeUp 0.35s ease 0.05s both}
.f2{animation:fadeUp 0.35s ease 0.12s both}
.f3{animation:fadeUp 0.35s ease 0.19s both}
.f4{animation:fadeUp 0.35s ease 0.26s both}
.f5{animation:fadeUp 0.35s ease 0.33s both}
`;
const styleEl = document.createElement("style");
styleEl.textContent = GLOBAL_CSS;
document.head.appendChild(styleEl);

/* ─── TRAINING DATA (71 real Hacettepe patients) ─────────────────────────── */
...
const DEFAULT_CREDS = { u: "doktor", p: "sculpt2024" };
...
/* ─── FLAG BUILDER ───────────────────────────────────────────────────────── */
function getFlags(answers, cat) {
  const flags = [];
  if (answers.motivation === "Başka insanların yorumları beni kötü etkiliyor" || answers.motivation === "Hayatımda büyük bir değişime ihtiyacım var")
    flags.push("Dış kaynaklı motivasyon");
  if (answers.expectation === "Tamamen farklı görünmek istiyorum")
    flags.push("Yüksek beklenti düzeyi");
  if (answers.multiDoctor === "Birçok doktora danıştım")
    flags.push("Birden fazla doktor görüşmesi");
  if (answers.patience === "Hızlı sonuç görmek istiyorum")
    flags.push("Hızlı sonuç beklentisi");
  if (answers.support === "Bu işleme karşılar" || answers.support === "Kimseye söylemedim")
    flags.push("Sosyal destek belirsiz");
  if (answers.revision === "Kusursuz sonuç bekliyorum")
    flags.push("Kusursuz sonuç beklentisi");
  if (cat === "green" || cat === "ambassador") {
    if (answers.motivation === "Görünümümü iyileştirmek istiyorum") flags.push("İçsel motivasyon");
    if (answers.expectation === "Doğal ve dengeli bir sonuç bekliyorum" || answers.expectation === "Küçük iyileştirmeler yeterli") flags.push("Gerçekçi beklentiler");
    if (answers.support === "Evet") flags.push("Ailesi destekliyor");
  }
  if (cat === "ambassador") {
    if (answers.socialMedia === "Sık paylaşırım") flags.push("Yüksek sosyal etki potansiyeli");
    if (answers.recommends === "Evet, sık öneririm") flags.push("Aktif tavsiye eğilimi");
  }
  return flags.slice(0, 2);
}

/* ─── KEY SIGNALS ────────────────────────────────────────────────────────── */
function getSignals(answers, cat) {
  if (cat === "red" || cat === "amber") {
    return [
      { label: "Motivasyon", val: answers.motivation?.split(" ").slice(0,3).join(" ") || "—" },
      { label: "Beklenti", val: answers.expectation?.split(" ").slice(0,3).join(" ") || "—" },
      { label: "Önceki Danışma", val: answers.multiDoctor || "—" },
    ];
  }
  if (cat === "ambassador") {
    return [
      { label: "Sosyal Medya", val: answers.socialMedia || "—" },
      { label: "Tavsiye Eğilimi", val: answers.recommends || "—" },
      { label: "Ek Prosedür", val: answers.crossSell || "—" },
    ];
  }
  return [
    { label: "Motivasyon", val: answers.motivation?.split(" ").slice(0,3).join(" ") || "—" },
    { label: "Beklenti", val: answers.expectation?.split(" ").slice(0,3).join(" ") || "—" },
    { label: "Sosyal Destek", val: answers.support || "—" },
  ];
}
...
function Sidebar() {
  ...
  const ic = (active) => ({ width:36,height:36,borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,color:active?"#60a5fa":"rgba(255,255,255,0.28)",background:active?"rgba(59,130,246,0.18)":"transparent",cursor:"pointer" });
  const sep = { width:20,height:1,background:"rgba(255,255,255,0.07)",margin:"6px 0" };
  return (
    <aside style={s}>
      <div style={logo}>S</div>
      <div style={ic(true)}>◉</div>
      <div style={ic(false)}>◈</div>
      <div style={ic(false)}>◎</div>
      <div style={sep}/>
      <div style={ic(false)}>◻</div>
      <div style={{flex:1}}/>
...
function PatientCard({ patient, onDelete }) {
  ...
  const cls = classify(patient.riskScore, patient.answers);
  const flags = getFlags(patient.answers, cls.cat);
  const signals = getSignals(patient.answers, cls.cat);

  const cardStyle = {
    background:"white",borderRadius:12,border:`1.5px solid ${open ? "#0c1428" : "#f1f3f5"}`,
    marginBottom:8,overflow:"hidden",cursor:"pointer",
    transition:"all 0.15s",
    boxShadow:open?"0 4px 18px rgba(12,20,40,0.09)":"none",
  };
    ...
    <div style={cardStyle}>
      <div style={rowStyle} onClick={() => setOpen(o => !o)}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,fontWeight:500,color:"#0c1428",marginBottom:2}}>{patient.answers.name || "İsimsiz Hasta"}</div>
          <div style={{fontSize:11,color:"#9ca3af"}}>{patient.answers.age && `${patient.answers.age} yaş · `}{patient.answers.procedure}</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:4,minWidth:148}}>
          {flags.map((f,i) => (
            <div key={i} style={{fontSize:11,color:"#6b7280",display:"flex",alignItems:"center",gap:5}}>
              <div style={{width:5,height:5,borderRadius:"50%",background:cls.color,flexShrink:0}}/>
              {f}
            </div>
          ))}
        </div>
        <div style={{fontSize:10,color:"#d1d5db",flexShrink:0}}>{patient.date?.split(" ")[0]}</div>
          ...
          <div style={{borderRadius:12,border:"1.5px solid #f1f3f5",overflow:"hidden"}}>
            ...
            <div style={{borderTop:"1px solid #f3f4f6",padding:"12px 16px",background:"white"}}>
              <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8}}>
                ...
                {patient.aiLoading && <div style={{fontSize:10,color:"#9ca3af",animation:"pulse 1.5s infinite"}}>Hazırlanıyor...</div>}
              </div>
              <div style={{fontSize:12,color:"#475569",lineHeight:1.7}}>{patient.aiText || (patient.aiLoading ? "Klinik analiz hazırlanıyor..." : "Analiz mevcut değil.")}</div>
            </div>
            {/* CTAs */}
            <div style={{borderTop:"1px solid #f3f4f6",padding:"10px 16px",display:"flex",gap:8,background:"white"}}>
              ...
              {cls.ambassador && <button onClick={(e)=>{e.stopPropagation();}} style={{flex:1,padding:"8px",borderRadius:8,fontSize:12,fontWeight:500,border:"1.5px solid #ddd6fe",background:"white",color:"#7c3aed"}}>🌟 Sadakat Programı</button>}
              ...
              {!showConfirm
                ? <button onClick={(e)=>{e.stopPropagation();setShowConfirm(true);}} style={{padding:"8px 12px",borderRadius:8,fontSize:12,border:"1.5px solid #fee2e2",background:"white",color:"#ef4444"}}>Sil</button>
...
/* ─── DOCTOR PANEL ───────────────────────────────────────────────────────── */
function DoctorPanel({ patients, setPatients, creds, onLogout }) {
  ...
  const todayPatients = patients.filter(p => p.date?.startsWith(todayStr.split(".").reverse().join("-")) || p.date?.includes(todayStr));
  const allPatients = patients;

  const redCount = allPatients.filter(p => p.riskScore >= 68).length;
  const totalCount = allPatients.length;
  const convRate = totalCount > 0 ? Math.round((allPatients.filter(p => p.riskScore < 68).length / totalCount) * 100) : 0;

  const displayList = (filter === "all" ? allPatients : allPatients.filter(p => {
    const cls = classify(p.riskScore, p.answers);
    return cls.cat === filter;
  }));

  const clinicalList = displayList.filter(p => !classify(p.riskScore, p.answers).ambassador);
  const ambassadorList = displayList.filter(p => classify(p.riskScore, p.answers).ambassador);

  function deleteRecord(id) {
    const updated = patients.filter(p => p.id !== id);
    store.set("sculptai:records", updated);
    setPatients(updated);
  }

  function clearAll() {
    store.del("sculptai:records");
    setPatients([]);
  ...
  function saveNewCreds() {
    ...
    store.set("sculptai:creds", { u: newU.trim(), p: newP });
    setShowChangePw(false); setNewU(""); setNewP(""); setNewP2(""); setPwErr("");
  }

  const panelStyle = { display:"flex",height:"100vh",overflow:"hidden",fontFamily:"'DM Sans',sans-serif" };
  const mainStyle = { flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:"#f7f8fa" };
  const topStyle = { padding:"20px 28px 0",display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexShrink:0,background:"white",borderBottom:"1px solid #e5e7eb",paddingBottom:16 };
  ...
  return (
    <div style={panelStyle}>
      ...
      <div style={mainStyle}>
        {/* TOP */}
        <div style={topStyle} className="f1">
          <div>
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:26,color:"#0c1428",fontWeight:400,letterSpacing:"-0.02em"}}>
              Günaydın, <span style={{color:"#3b82f6"}}>{creds.name || "Doktor"}</span>
            </div>
            <div style={{fontSize:12,color:"#9ca3af",marginTop:3}}>{today} · {todayPatients.length > 0 ? `Bugün ${todayPatients.length} yeni kayıt` : "Bugün henüz kayıt yok"}</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button onClick={() => setShowChangePw(v => !v)} style={{padding:"6px 14px",borderRadius:8,fontSize:12,border:"1.5px solid #e5e7eb",background:"white",color:"#6b7280"}}>⚙ Şifre</button>
            ...
            <div style={{width:34,height:34,borderRadius:"50%",background:"linear-gradient(135deg,#3b82f6,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:600,color:"white"}}>
              {creds.name ? creds.name.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase() : "DR"}
            </div>
          </div>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 28px 24px"}}>
          ...
          {/* CHANGE PW */}
          {showChangePw && (
            <div style={{background:"white",border:"1px solid #e5e7eb",borderRadius:12,padding:"16px 20px",marginBottom:18,animation:"fadeUp 0.25s ease"}} className="f1">
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {[["Yeni kullanıcı adı",newU,setNewU,"text"],["Yeni şifre",newP,setNewP,"password"],["Şifre tekrar",newP2,setNewP2,"password"]].map(([ph,val,set,type]) => (
                  <input key={ph} type={type} placeholder={ph} value={val} onChange={e=>set(e.target.value)}
                    style={{flex:1,minWidth:130,padding:"9px 12px",background:"#f9fafb",border:"1.5px solid #e5e7eb",borderRadius:9,color:"#111827",fontSize:13,outline:"none"}}/>
                ))}
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:24}} className="f2">
            {[
              {val:redCount,label:"Dikkat Gerektiriyor",note:`↑ Bugün ${todayPatients.filter(p=>p.riskScore>=68).length} yeni`,color:"#ef4444",grad:"linear-gradient(90deg,#ef4444,#f97316)"},
              {val:convRate+"%",label:"Uygun Görünüyor Oranı",note:"Toplam değerlendirmede",color:"#3b82f6",grad:"linear-gradient(90deg,#3b82f6,#06b6d4)"},
              {val:totalCount,label:"Toplam Hasta",note:`Bu ay ${patients.filter(p=>{const d=new Date(p.createdAt||Date.now());return d.getMonth()===new Date().getMonth()}).length} eklendi`,color:"#10b981",grad:"linear-gradient(90deg,#10b981,#06b6d4)"},
            ].map(k => (
              <div key={k.label} style={{background:"white",border:"1.5px solid #f1f3f5",borderRadius:12,padding:"18px 20px",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{fontSize:11,color:"#9ca3af",letterSpacing:"0.05em",marginBottom:6}}>{k.label}</div>
          ...
          {/* CLINICAL LIST */}
          <div className="f4">
            {clinicalList.length === 0 && ambassadorList.length === 0 && (
              <div style={{textAlign:"center",padding:"60px 20px",color:"#9ca3af"}}>
                <div style={{fontSize:40,marginBottom:14}}>📋</div>
                <div style={{fontSize:15,color:"#374151",marginBottom:8}}>Henüz kayıt yok</div>
                <div style={{fontSize:13}}>Hastalar formu doldurdukça burada görünecek</div>
              </div>
            )}
            {clinicalList.map(p => <PatientCard key={p.id} patient={p} onDelete={deleteRecord}/>)}
          </div>
          ...
          {ambassadorList.length > 0 && (
            <div className="f5">
              ...
              {ambassadorList.map(p => <PatientCard key={p.id} patient={p} onDelete={deleteRecord}/>)}
            </div>
          )}
...
/* ─── PATIENT FORM ───────────────────────────────────────────────────────── */
function PatientForm({ model, trainPct, doctorId, onSubmit }) {
  ...
  const C = { bg:"#f7f8fa", accent:"#3b82f6", navy:"#0c1428", muted:"#9ca3af", border:"#e5e7eb", card:"white" };
  ...
  if (submitted) return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{textAlign:"center",animation:"fadeUp 0.6s ease",maxWidth:440,padding:24}}>
        ...
        <button onClick={()=>{setSubmitted(false);setAnswers({});setCurrentQ(0);}}
          style={{padding:"11px 28px",background:"white",border:"1.5px solid",borderColor:C.border,borderRadius:11,color:C.muted,fontSize:13,cursor:"pointer"}}>
          Yeni Form Doldur
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans',sans-serif",color:C.navy}}>
      ...
      <main style={{maxWidth:580,margin:"0 auto",padding:"36px 20px"}}>
        ...
        {/* Question card */}
        <div style={{background:C.card,border:`1.5px solid ${C.border}`,borderRadius:14,padding:"24px 22px",marginBottom:14}} className="f3">
          ...
          {q.type==="radio" && (
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
              {q.options.map(opt => {
                ...
                return (
                  <button key={opt} onClick={()=>setAnswers(p=>({...p,[q.id]:opt}))} style={{padding:"12px 14px",background:sel?"#eff6ff":"#f9fafb",border:`1.5px solid ${sel?C.accent:C.border}`,borderRadius:10,color:sel?C.accent:"#374151",fontSize:13,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:11,transition:"all 0.15s"}}>
...
function Login({ onLogin }) {
  ...
  function attempt() {
    const saved = store.get("sculptai:creds") || DEFAULT_CREDS;
    if (u === saved.u && p === saved.p) onLogin(saved);
    ...
    <div style={{minHeight:"100vh",background:"#f7f8fa",fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{width:"100%",maxWidth:360}}>
        ...
        <div style={{background:"white",border:`1.5px solid ${C.border}`,borderRadius:16,padding:"26px 24px"}}>
          {[["Kullanıcı Adı",u,setU,"text"],["Şifre",p,setP,"password"]].map(([label,val,set,type]) => (
            <div key={label} style={{marginBottom:14}}>
              ...
              <input type={type} value={val} onChange={e=>set(e.target.value)} onKeyDown={e=>e.key==="Enter"&&attempt()}
                style={{width:"100%",padding:"11px 14px",background:"#f9fafb",border:`1.5px solid ${C.border}`,borderRadius:10,color:C.navy,fontSize:14,outline:"none"}}/>
            </div>
          ))}
          ...
          <button onClick={attempt} style={{width:"100%",padding:"12px",background:"linear-gradient(135deg,#3b82f6,#0c1428)",border:"none",borderRadius:11,color:"white",fontSize:14,fontWeight:600,cursor:"pointer"}}>Giriş Yap →</button>
          <div style={{marginTop:14,padding:"9px 13px",background:"#f9fafb",border:`1.5px solid ${C.border}`,borderRadius:8,fontSize:11,color:C.muted}}>Varsayılan: <strong style={{color:C.navy}}>doktor</strong> / <strong style={{color:C.navy}}>sculpt2024</strong></div>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  const [view, setView] = useState("patient"); // patient | login | doctor
  ...
  // Load from storage
  useEffect(() => {
    const r = store.get("sculptai:records"); if (r) setPatients(r);
    const c = store.get("sculptai:creds"); if (c) setCreds(c);
  }, []);
  ...
  function handleFormSubmit(answers) {
    ...
    const updated = [rec, ...patients];
    store.set("sculptai:records", updated);
    setPatients(updated);
    fetchAI(answers, score, cls, rec.id, updated);
  }

  async function fetchAI(a, score, cls, recId, currentList) {
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514", max_tokens: 700,
          messages: [{ role: "user", content: `You are SculptAI, trained on 71 real Hacettepe University plastic surgery patients. Write a brief clinical observation IN ENGLISH (3-4 sentences max, no headers, no bullet points). Soft advisory tone — never prescriptive. Mention 1-2 specific risk signals and 1 positive. End with a communication tip.

Patient: ${a.name||"Anonymous"}, Age ${a.age}, ${a.gender} | Procedure: ${a.procedure}
Motivation: ${a.motivation} | Expected: ${a.expectation} | Prev surgery: ${a.prevSurgery}
Multi-doctor: ${a.multiDoctor} | Risk knowledge: ${a.riskKnowledge} | Patience: ${a.patience}
...
ML RISK: ${score}/100 | ASSESSMENT: ${cls.label}` }]
      ...
      setPatients(prev => {
        const updated = prev.map(p => p.id === recId ? { ...p, aiText: txt, aiLoading: false } : p);
        store.set("sculptai:records", updated);
        return updated;
      });
    } catch {
      setPatients(prev => prev.map(p => p.id === recId ? { ...p, aiText: "Sistem gözlemi şu an kullanılamıyor.", aiLoading: false } : p));
    }
  }

  // Detect doctor route from URL
  useEffect(() => {
    const path = window.location.pathname;
    if (path.startsWith("/panel")) setView("login");
  }, []);

  if (view === "patient") return (
    <div>
      <PatientForm model={model} trainPct={trainPct} onSubmit={handleFormSubmit} />
      <button onClick={() => setView("login")} style={{ position:"fixed",bottom:16,right:16,padding:"6px 14px",background:"rgba(12,20,40,0.06)",border:"1px solid rgba(12,20,40,0.1)",borderRadius:8,color:"rgba(12,20,40,0.4)",fontSize:11,cursor:"pointer",backdropFilter:"blur(4px)" }}>🔒</button>
    </div>
  );

  if (view === "login") return (
    <Login onLogin={(c) => { setCreds(c); setView("doctor"); }} />
  );
